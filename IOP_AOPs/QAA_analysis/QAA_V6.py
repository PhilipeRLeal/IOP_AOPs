...
def get_aw_lambda(lambda_nm=412, verbose=True):
    ...
    if lambda_nm in aw_lambdas.index.values:
        ...
        nearest_aw_lambda_0 = get_nearest_column(list(aw_lambdas.index), lambda_nm)
        
        if verbose:
            print('getting nearest water absortion coeficient for lambda {}'.format(lambda_nm))
            print('Lambda selected for aw(lambda0): {0}\n'.format(   str(nearest_aw_lambda_0)) )
...
def get_rrs_lambda (Rrs_lambda):
    ...
    '''
    Description: 
        This function return the remote sensing surface reflectance (L/E), also known as 'rrs'
...
def get_total_absortion_coeficient_lambda_0(Rrs_lambda_670, 
                                   ...
                                   H_exponent_coefficients=dict(h0=-1.145902928  , # values from Lee
                                                                h1=-1.365828264  , # values from Lee
                                                                h2=-0.469266028),   # values from Lee
                                   verbose=False,
                                   ):
    
    '''
    Constants in H_exponent_coefficients were the average of the coefficients obtained by least-square fitting
        a(λ0) of the synthetic data set adopted by the IOCCG [2006] for SeaWiFS, MODIS, and
        MERIS bands. 
        
    In short, one set of parameters is proposed for the three sensors (good
        enough for comparison of derived IOPs with in situ measurements), except the change of
        aw(λ0) values for each sensor. 
    ...
    if Rrs_lambda_670 < 0.0015: # in sr ** (-1) units
        ...
        a_lambda_670 = a_water_lambda_0 + 0.39 * (Rrs_lambda_670/(Rrs_lambda_443 + Rrs_lambda_490))**1.14
...
def get_ag_lambda_443(a_lambda_412, a_lambda_443, tau, Csi, aw_lambda_412, aw_lambda_443):
...
def apply_QAA(Rrs_Data, lambda_sensor_step=1 ,  lambda_0=550,  verbose=True):


    '''
    Description:
        This function evaluates the QAA algorithm over a pandas dataframe of reflectances.
    ...
    Input:
        ...
        lambda_sensor_step: it is the distance between the spectral bands applied in the dataframe. 
                            It is equal to one, for standard.
    ...
    Returns 
        ...
        Each key contains all values of that given QAA coefficient per spectral band.
            
    '''

    
    lambda_range = np.arange(Rrs_Data.columns.min(), 
                             Rrs_Data.columns.max()+1, lambda_sensor_step).astype(int)
    
    aw_lambda = pd.Series([get_aw_lambda(x) for x in lambda_range], index=lambda_range)
    ...
    a_tot_lambda_0 = Rrs_Data.apply(lambda x: get_total_absortion_coeficient_lambda_0(Rrs_lambda_670=x[nearest_670_lambda], 
    ...
    bbp_lambda_0 = pd.Series(get_bbp_lambda_0(u_lambda_0,
                     a_tot_lambda_0,
                     bbw_lambda_670,
                     Rrs_lambda_670,
                     Reflectance_threshold_for_670 = 0.0015,
    ...
    Ni = get_Ni(bbp_lambda_0, rrs_data[nearest_443_lambda], rrs_data[nearest_550_lambda] )
    
    lambdas_to_bbp =  lambda_range
    
    
    
    bbp_lambda = pd.concat([pd.Series(get_bbp_lambda (bbp_lambda_0, lambda_0, x, Ni), name=x) for x in lambdas_to_bbp], axis=1)
    
    lambdas_to_bbw =  lambda_range

    
    bbw_lambda = pd.Series(get_bbw_lambda(lambdas_to_bbw) , index=lambdas_to_bbw)
    
    
    a_lambda = get_a_lambda(u_lambda, bbw_lambda[Rrs_Data.columns], bbp_lambda[Rrs_Data.columns])
    ...
    adg_lambda = pd.concat([get_adg_lambda(ag_lambda, S, C , lambda_0=443) for C in Rrs_Data.columns], axis=1)
    
    adg_lambda.columns = Rrs_Data.columns
    
    aph_lambda = get_aph_lambda(a_lambda , adg_lambda, aw_lambda)[Rrs_Data.columns] 
